...
class PF:
    ...
    def channel_realloc(self, net1=Net):
        ...
        for i in range(b.BS_num):                                    # 参考net类中信道分配的函数
            for j in range(b.FrequencyBand_num * b.Subcarrier_num):  # 对子载波进行分配，初始化
                ...
                while UE_order_val[i][j][m] > 0:  # 当前序号的子载波非零，即表示用户与该子载波是可相连的
                    ...
                    if BandNumConnectedToUE[k] < b.Max_BandNumConnectedToUE and (k in self.BS_UE_priority[i]):  # 用户连接子载波数未达到最大值
        ...
        b.sinr = np.zeros((b.BS_num, b.FrequencyBand_num *b.Subcarrier_num))
        b.rate_for_UE = np.zeros(b.UE_num)
        return b


    def recompute_SINR_on_bs_sub(self, n, k, net1=Net):
        c = net1
        if c.SN_BandToUE[n][k] == 0:
            sinr = 0
            '''应该在哪定义？'''                                               # 下面的power应该为数组，应该修改
        else:
            power_on_sub = c.BS_max_power/(c.FrequencyBand_num*c.Subcarrier_num)   # 单个子载波功率大小，初始平均分配，之后用drl方法
        ...
        for i in range(c.BS_num):
            for j in range(c.FrequencyBand_num*c.Subcarrier_num):
                if c.SN_BandToUE[i, j]!= 0:
                    k = c.SN_BandToUE[i, j]
                    c.rate_for_UE[k] += bandwidth * log2(1+c.sinr[i, j])
